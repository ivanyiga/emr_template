<template>
    <div class="my-6">
        <h3 class="text-lg font-medium leading-6 text-gray-900">Encounters Stats</h3>
        <dl class="mt-5 grid grid-cols-1 divide-y divide-gray-200 overflow-hidden rounded-lg bg-white shadow md:grid-cols-3 md:divide-y-0 md:divide-x">
            <div v-for="item in stats" :key="item.name" class="px-4 py-5 sm:p-6">
                ...
                <dd class="mt-1 flex items-baseline justify-between md:block lg:flex">
                    <div class="flex items-baseline text-2xl font-semibold primary-text">
                        {{ item.stat }}
...
<script setup>
    ...
    const stats = [
        { name: 'Total Encounters Today', stat: '71,897', previousStat: '70,946', change: '12%', changeType: 'increase' },
        { name: 'Encounters Last Week', stat: '58.16%', previousStat: '56.14%', change: '2.02%', changeType: 'increase' },
        { name: 'Total Encounters (Prev 30 days)', stat: '24.57%', previousStat: '28.62%', change: '4.05%', changeType: 'decrease' },
    ]
</script>
